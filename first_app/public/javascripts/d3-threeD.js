...
function transformSVGPath(pathStr) {
  ...
  while (idx <= len) {
    ...
    switch (activeCmd) {
      ...
      case 'H':
      case 'V':
        if (prevxs.indexOf(x) >= 0 && prevys[prevxs.indexOf(x)] == y){
          duplicate = true
        }
        prevxs.push(x)
        prevys.push(y)
        ...
        if (prevxs.indexOf(nx) >= 0 && prevys[prevxs.indexOf(nx)] == ny){
          console.log("path has duplicate point")
        }
        path.lineTo(nx, ny);
        x = nx;
        y = ny;
